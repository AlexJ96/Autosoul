<?php 
    ...
    try {
        ...
        foreach($stmt->fetchAll() as $k=>$v) {
            $title = $v['title'];
            $subTitle = $v['sub-title'];
            $articleContent = $v['article-content'];
            $featuredImage = $v['featured-image'];
        }

        $stmt = Config::get()->prepare("SELECT * FROM `article_tags` WHERE id = :id");
        $stmt->bindParam(":id", $_GET["id"]);
        $stmt->execute();
    
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $amount = 0;
        foreach($stmt->fetchAll() as $k=>$b) {
            $tags[$amount] = $b['tag'];
            $amount++;
        }

        $stmt = Config::get()->prepare("SELECT * FROM `article_images` WHERE article_id = :id");
        $stmt->bindParam(":id", $_GET["id"]);
        $stmt->execute();
    
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        $imageAmount = 0;
        foreach($stmt->fetchAll() as $k=>$n) {
            $imageGallery[$imageAmount] = $n['url'];
            $imageAmount++;
        }
...
<html>
    <head>
        ...
        <script type="text/javascript">
            ...
            var featuredImage = "<?php echo $featuredImage; ?>";
            var tags = [ ];
            var imageGallery = [ ];
            var amount = "<?php echo $amount; ?>";
            var imageAmount = "<?php echo $imageAmount; ?>";

            $(function() {
                $('#title').val(title);
                $('#subTitle').val(subTitle);
                $('#markItUp').val(articleContent);
                ...
                <?php
                for ($b = 0; $b < $imageAmount-1; $b++) {
                    echo "imageGallery[$b] = '" . $imageGallery[$b] . "';";
                }
                ?>

                for (var i = 0; i < imageAmount-1; i++) {
                    addImage(imageGallery[i]);
                }
            });
            ...
            <div class="admin-article">
                <div id="schedule">
                    ...
                    <div class="schedule-buttons">
                        ...
                        <div class="schedule-schedule">
                            <p class="schedule-button-popup-edit"> Schedule </p>
                        </div>
                    </div>
                ...
                <div class="publish-content">
                    <div class="publish-options">
                        <ul>
                            <li class="publish-active"> <a href="/autosoul/admin/edit?id="> Edit</a></li>
                    ...
                    <div class="publish-area">
                        ...
                        <div class="publish-sub-title">
                            <input id="subTitle" name="sub-title" type="text" placeholder="Enter sub title">
                        </div>
                    ...
                    <div class="publish-side-panel">
                        <div class="publish-buttons">
                            <img class="post-article-edit" src="../resources/images/admin/post.png" />
                            <img class="schedule-button" src="../resources/images/admin/schedule-main.png" />
                            <img class="draft-button-edit" src="../resources/images/admin/draft.png" />
                        </div>
